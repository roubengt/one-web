...
const App = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const secciones = ['inicio', 'nosotros', 'planes', 'profesores', 'instagram', 'reglamento', 'contacto']
    ...
    <div style={{ backgroundColor: BG, color: 'white', overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .terminos-btn:hover { color: white !important; }
      ...
      <div style={{ position: 'fixed', top: 0, left: menuAbierto ? 0 : '-100%', width: '300px', height: '100vh', backgroundColor: '#060606', borderRight: `1px solid ${BORDER}`, zIndex: 999, transition: 'left 0.4s cubic-bezier(0.4, 0, 0.2, 1)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '48px' }}>
        ...
        <div style={{ marginTop: '48px', display: 'flex', gap: '20px', alignItems: 'center' }}>
          <a href="https://www.facebook.com/oneevolution.cl/" target="_blank" rel="noreferrer" className="sidebar-social" onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.color = 'white'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.color = '#aaa'}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
          </a>
          <a href="https://www.instagram.com/oneevolution.cl" target="_blank" rel="noreferrer" className="sidebar-social" onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.color = 'white'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.color = '#aaa'}>
          ...
          <a href="https://wa.me/56956723743" target="_blank" rel="noreferrer" className="sidebar-social" onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.color = 'white'} onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.color = '#aaa'}>
      ...
      <section id="inicio" style={{ height: '100vh', position: 'relative', overflow: 'hidden', backgroundColor: '#000' }}>
        ...
        {!videoSrc && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <h1 style={{ ...T, fontSize: 'clamp(80px, 20vw, 200px)', color: 'white', letterSpacing: '0.15em', lineHeight: 0.9, textAlign: 'center' }}>ONE</h1>
            <p style={{ ...B, fontSize: 'clamp(10px, 1.5vw, 14px)', letterSpacing: '0.5em', color: '#999', textTransform: 'uppercase', marginTop: '16px', fontWeight: 500 }}>YOUR EVOLUTION</p>
          </div>
        )}
        ...
        {webData.profesores.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '32px', maxWidth: '1100px', margin: '0 auto' }}>
            {webData.profesores.map((prof, i) => (
              <div key={i} className="prof-card" style={{
                borderRadius: '16px',
                overflow: 'hidden',
                backgroundColor: '#111',
                border: '1px solid #222',
                boxShadow: '0 4px 24px rgba(0,0,0,0.3)',
                opacity: seccionVisible['profesores'] ? 1 : 0,
                transform: seccionVisible['profesores'] ? 'translateY(0)' : 'translateY(40px)',
                transition: `opacity 0.6s ease ${i * 0.1}s, transform 0.6s ease ${i * 0.1}s, border-color 0.3s, box-shadow 0.3s`
              }}>
                <div style={{ width: '100%', aspectRatio: '4/3', overflow: 'hidden', backgroundColor: '#0a0a0a', position: 'relative' }}>
                  {prof.foto ? (
                    <img src={prof.foto} alt={prof.nombre} className="prof-img" style={{
                      width: '100%', height: '100%', objectFit: 'cover',
                      filter: 'grayscale(60%) brightness(0.8)',
                      transition: 'filter 0.4s ease, transform 0.4s ease',
                    }} />
                  ) : (
                    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a1a1a 0%, #111 100%)' }}>
                      <span style={{ ...T, fontSize: '72px', color: '#333' }}>{prof.nombre.charAt(0).toUpperCase()}</span>
                    </div>
                  )}
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '60px', background: 'linear-gradient(to top, #111 0%, transparent 100%)' }} />
                </div>
                <div style={{ padding: '24px 28px 28px' }}>
                  <h3 style={{ ...T, fontSize: '24px', color: 'white', marginBottom: '6px', letterSpacing: '1px' }}>{prof.nombre}</h3>
                  <p style={{ ...B, fontSize: '10px', letterSpacing: '3px', color: '#666', marginBottom: prof.descripcion ? '16px' : '0', fontWeight: 500 }}>COACH PERSONAL</p>
                  {prof.descripcion && (
                    <p style={{
                      ...B,
                      fontSize: '14px',
      ...
      {/* INSTAGRAM */}
      {((webData as any).instagramPosts || []).length > 0 && (
        <section id="instagram" style={{ padding: 'clamp(60px, 8vw, 120px) clamp(20px, 5vw, 80px)', backgroundColor: BG }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', maxWidth: '1100px', margin: '0 auto' }}>
            {((webData as any).instagramPosts || []).map((url: string, i: number) => {
              const postId = url.match(/\/p\/([^/?]+)/)?.[1] || ''
              return (
                <a key={i} href={url} target="_blank" rel="noreferrer" className="plan-card" style={{
                  borderRadius: '16px',
                  overflow: 'hidden',
                  backgroundColor: '#111',
                  border: '1px solid #222',
                  ...
                  transition: `opacity 0.6s ease ${i * 0.15}s, transform 0.6s ease ${i * 0.15}s, border-color 0.3s, box-shadow 0.3s`,
                }}>
                  <div style={{ width: '100%', aspectRatio: '1/1', overflow: 'hidden', backgroundColor: '#0a0a0a', position: 'relative' }}>
                    <img
                      src={`https://instagram.com/p/${postId}/media/?size=l`}
                      alt={`Post ${i + 1}`}
                      style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.85)', transition: 'transform 0.4s ease, filter 0.4s ease' }}
                      className="prof-img"
                      onError={(e) => {
                        (e.target as HTMLImageElement).style.display = 'none'
                        ;(e.target as HTMLImageElement).parentElement!.innerHTML = `<div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,#1a1a1a,#111)"><svg width="48" height="48" viewBox="0 0 24 24" fill="#333"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg></div>`
                      }}
                    />
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '80px', background: 'linear-gradient(to top, #111 0%, transparent 100%)' }} />
                  </div>
                  <div style={{ padding: '20px 24px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="#bbb"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                      <span style={{ ...B, fontSize: '13px', color: '#bbb', fontWeight: 400 }}>@oneevolution.cl</span>
                    </div>
                    <span style={{ ...B, fontSize: '11px', color: '#555', fontWeight: 300, letterSpacing: '2px' }}>VER POST →</span>
                  </div>
                </a>
              )
            })}
      ...
      {modalTerminos && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.92)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ backgroundColor: '#111', border: `1px solid ${BORDER}`, borderRadius: '4px', width: '100%', maxWidth: '860px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 28px', borderBottom: `1px solid ${BORDER}` }}>
              <div>
                <p style={{ ...B, fontSize: '9px', letterSpacing: '4px', color: '#555', marginBottom: '4px', fontWeight: 500 }}>ONE EVOLUTION</p>
                <h2 style={{ ...T, fontSize: '22px', color: 'white' }}>TÉRMINOS Y CONDICIONES</h2>
              </div>
              <button onClick={() => setModalTerminos(false)} style={{ background: 'none', border: `1px solid ${BORDER}`, color: '#555', fontSize: '18px', cursor: 'pointer', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '2px', transition: 'all 0.2s', flexShrink: 0 }}
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = BORDER; (e.currentTarget as HTMLButtonElement).style.color = '#555' }}>
                ✕
              </button>
            </div>
            <div style={{ flex: 1, overflow: 'auto' }}>
              {(webData as any).terminosPdf ? (
                <iframe src={(webData as any).terminosPdf} style={{ width: '100%', height: '70vh', border: 'none', display: 'block' }} title="Términos y Condiciones ONE Evolution" />
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '300px', gap: '16px' }}>
                  <p style={{ ...B, fontSize: '14px', color: '#444', fontWeight: 300 }}>Los términos y condiciones aún no han sido cargados.</p>
                  <p style={{ ...B, fontSize: '12px', color: '#333', fontWeight: 300 }}>El administrador puede subirlos desde el panel de administración.</p>
                </div>
              )}
            </div>
            <div style={{ padding: '16px 28px', borderTop: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setModalTerminos(false)} style={{ padding: '10px 28px', backgroundColor: 'white', border: 'none', color: '#000', fontSize: '11px', letterSpacing: '2px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontWeight: 600, borderRadius: '2px' }}>
                CERRAR
              </button>
            </div>
          </div>
      ...
      <footer style={{ backgroundColor: '#000', borderTop: '1px solid #0d0d0d', padding: '32px 48px', textAlign: 'center' }}>
        ...
        <p style={{ ...B, fontSize: '11px', color: '#555', fontWeight: 400, marginBottom: '10px' }}>
          © 2025 ONE Evolution. Todos los derechos reservados. | www.oneevolution.cl
        </p>
        <button onClick={() => setModalTerminos(true)} className="terminos-btn"
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontWeight: 300, fontSize: '11px', color: '#333', textDecoration: 'underline', transition: 'color 0.2s' }}>
